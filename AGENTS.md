...
| `PLANNOTATOR_PLAN_KEYPRESS` | Set to `0` / `false` to disable tmux plan-dialog keypress injection (Claude Code only). Claude Code 2.1.199–2.1.207 ignore the PermissionRequest `allow` for ExitPlanMode — the interactive plan dialog waits for a keypress — so under tmux the hook injects the approval key (`tmux send-keys`) on a plain approve to make the plan cascade (e.g. after a headless multi-LLM auto-approve). Default: enabled. Disable if a future Claude Code honors the allow again. Also settable via `~/.plannotator/config.json` (`{ "planKeypress": false }`). Set `PLANNOTATOR_PLAN_KEYPRESS_DEBUG=1` to trace the decision to `<dataDir>/debug/plan-keypress.log`. See `packages/server/plan-dialog.ts`. |
