...
const AppInner: React.FC = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (allAnnotations.length > 0 || codeAnnotations.length > 0 || globalAttachments.length > 0 || hasDocAnnotations || editorAnnotations.length > 0 || editsSection || savedChangesSection) {
        body.feedback = getCurrentFeedbackPayload(checkedSavedFileChanges) + hookText;
      } else if (hookText) {
  ...
  const handleDeny = async () => {
    ...
    try {
      ...
      await fetch('/api/deny', {
        ...
        body: JSON.stringify({
          draftGeneration: getDraftGeneration(),
          feedback: getCurrentFeedbackPayload(checkedSavedFileChanges) + formatPromptHooks(activeHooks, automationsList),
          planSave: {
            enabled: planSaveSettings.enabled,
            ...(planSaveSettings.customPath && { customPath: planSaveSettings.customPath }),
          },
  ...
  // Automation quick-send — deny with a preset feedback payload from the
  // AutomationsDropdown, bypassing the annotation flow.
  ...
  return (
    <ThemeProvider defaultTheme="dark">
      ...
      <div data-print-region="root" className="h-screen flex flex-col bg-background overflow-hidden">
              ...
              {!goalSetupMode && !isPlanDiffActive && !archive.archiveMode && !isEditingMarkdown && !(isHtmlSurface && htmlToolsHidden) && (
                <div
                  ...
                  className={isHtmlSurface
                    ...
                    : "w-full mb-3 md:mb-4 flex items-center justify-between"}
                ...
                >
                  ...
                  />
                  {!isHtmlSurface && !annotateMode && isApiMode && (
        ...
        {/* Completion overlay — skipped for auto-approve so the user can read the revised plan */}
        <CompletionOverlay
